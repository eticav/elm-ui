...
mouseEvent : Int->Int->Int->Rectangle->MouseEvent
mouseEvent clientX clientY buttons target =
    { clientPos = Position clientX clientY
    , targetPos = Position (truncate target.left) (truncate target.top)
    , buttons = buttons
    , width = truncate target.width
    , height = truncate target.height
    }
